      ...
      <aside style={{
        ...
        <div style={{ flex: 1, overflowY: 'auto', padding: '8px 0' }}>
          {items.length === 0 ? (
            <div style={{ display: 'flex', flexDirection: 'column', alignItems: 'center', justifyContent: 'center', height: '100%', gap: '18px', padding: '40px 28px', textAlign: 'center' }}>
              ...
              <button onClick={() => setCartOpen(false)} className="btn-gold" style={{ fontSize: '9px', cursor: 'none' }}>
        ...
        {items.length > 0 && (
          <div style={{ padding: '22px 28px 32px', borderTop: '1px solid var(--border)', flexShrink: 0 }}>
            ...
            <p style={{ fontFamily: 'var(--font-sans)', fontSize: '10px', color: 'var(--muted)', marginBottom: '20px' }}>Livraison (8 TND) calculée à la commande</p>
            <button onClick={() => { setCartOpen(false); setTimeout(() => setCheckoutOpen(true), 300); }} className="btn-gold" style={{ width: '100%', textAlign: 'center', padding: '16px', fontSize: '10px', cursor: 'none', display: 'block' }}>
              Passer la commande →
            </button>
            <button onClick={() => setCartOpen(false)} className="continue-btn"
              style={{ width: '100%', marginTop: '10px', padding: '13px', background: 'none', border: '1px solid var(--border)', fontFamily: 'var(--font-sans)', fontSize: '10px', letterSpacing: '.2em', textTransform: 'uppercase', color: 'var(--muted)', cursor: 'none', transition: 'all .3s' }}>
